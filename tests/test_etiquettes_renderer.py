...
def test_compute_strip_widths_returns_expected_dimensions():
    ...
    index 6 + ID 35 + 3 x Qn (17.5) + zone blanche (reste) + logo 25 = 277 mm."""
    from src.planrec.etiquettes_renderer import compute_strip_widths, LOGO_CELL_W_MM
    ...
    assert widths["logo"] == LOGO_CELL_W_MM
    expected_blank = 277.0 - 6.0 - 35.0 - (3 * 17.5) - LOGO_CELL_W_MM
    assert abs(widths["blank"] - expected_blank) < 0.01


def test_compute_strip_widths_logo_cell_fixed_width():
    """La cellule logo a une largeur fixe (Hager-like), la zone blanche
    absorbe la variation selon n_disjoncteurs."""
    from src.planrec.etiquettes_renderer import compute_strip_widths, LOGO_CELL_W_MM

    widths_3 = compute_strip_widths(n_disjoncteurs=3, page_usable_width_mm=277.0)
    widths_7 = compute_strip_widths(n_disjoncteurs=7, page_usable_width_mm=277.0)
    assert widths_3["logo"] == LOGO_CELL_W_MM
    assert widths_7["logo"] == LOGO_CELL_W_MM
    # La zone blanche doit rester strictement positive même à 7 disjoncteurs
    assert widths_7["blank"] > 0
...
def test_render_rcd_row_draws_expected_text_and_rects():
    ...
    assert "63 A · 30 mA · Type A" in text  # calibre + type + sensibilité de l'ID
